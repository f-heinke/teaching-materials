...
```{r}
dd_diag <- ________(_______, sep = "\t", _______ = _______)
head(dd_diag) # print the first 6 lines/elements of a table, matrix or vector or ....

```

```{r}
_______ <- _______(_______, sep = "\t", _______ = _______)
...
```{r}
dd_comb <- _______(dd_diag, dd_msmt)
head(dd_comb)
```
...
```{r}
match(dd_diag$_______, dd_msmt$_______)
```


```{r}
dd_diag$_______[_______]
dd_msmt$_______[_______]
...
```{r}
_______ <- match(dd_diag$_______, dd_msmt$_______)
```

Nutzen wir diese Indexreihenfolge, um den zweiten Datensatz entsprechend dem ersten zu sortieren.

```{r}
_______ <- dd_msmt[_______,] # ordered according to id list in diagnostics table
...
```{r}
dd_comb <- _______(dd_diag, dd_msmt_ordered)
head(dd_comb)
```
...
dd_comb2 <- cbind(dd_diag, dd_msmt_ordered[,_______])
head(dd_comb2)
```
...
```{r}
dd_comb2$_____ <- 2 * pi * dd_comb2$______

```

Ein einfachste Punktdiagramm zeigt einen interessanten Zusammenhang. Ein Hilfslinie (in blau hinzugefügt mit `abline`) dient der Illustration identischer Wertigkeit:
